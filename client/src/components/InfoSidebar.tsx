...
export function InfoSidebar({
  ...
  const [showCommandDetails, setShowCommandDetails] = useState<boolean>(false);
  const [showPerformanceDetails, setShowPerformanceDetails] = useState<boolean>(false);

  // UX state
  const [busy, setBusy] = useState<boolean>(false);
  const [typingBlink, setTypingBlink] = useState<boolean>(true); // blinking cursor for streaming
  ...
  useEffect(() => {
    if (!busy) return;
    const id = setInterval(() => setTypingBlink((b: boolean) => !b), 450);
  ...
  // Log whenever the external systemEnabled prop changes (e.g., GUI toggle or SSE)
  useEffect(() => {
    if (lastEnabledRef.current === null) {
      lastEnabledRef.current = systemEnabled;
      appendLog(`[SYSTEM] Status: ${systemEnabled ? 'Online ✅' : 'Offline ⛔'}`);
      return;
    }
    if (lastEnabledRef.current !== systemEnabled) {
      appendLog(`[SYSTEM] Status: ${systemEnabled ? 'Online ✅' : 'Offline ⛔'}`);
      lastEnabledRef.current = systemEnabled;
    }
  ...
  // Streaming helper: updates a specific line by index
  const updateLogAt = (index: number, text: string) =>
    setLogs((prev: string[]) => {
      const clone = [...prev];
      clone[index] = text;
      return clone;
  ...
  // Let the server do intent detection; allow sending while offline.
  const canSend = (cmd: string) => {
    if (busy) return false;
    return Boolean(cmd.trim());
  };

  // Build an authoritative system snapshot string from the server.
  async function buildSystemLine(fallbackEnabled: boolean) {
    try {
      const sysRes = await fetch(`${API_BASE}/api/system`, { cache: 'no-store' });
      const sysJson = await sysRes.json();
      // expected: { ok, enabled, version, updatedAt, serverTime }
      const enabled = Boolean(sysJson?.enabled);
      const version = sysJson?.version ?? '0';
      const updatedAt = sysJson?.updatedAt ?? new Date().toISOString();
      const serverTime = sysJson?.serverTime ?? new Date().toISOString();
      return {
        systemLine: `DASHBOARD_STATUS v${version} enabled=${enabled} updatedAt=${updatedAt} (serverTime=${serverTime})`,
        enabled,
      };
    } catch {
      // graceful fallback to client-side knowledge if server fetch fails
      return {
        systemLine: `DASHBOARD_STATUS enabled=${fallbackEnabled} (clientFallback=true ts=${new Date().toISOString()})`,
  ...
  const handleSend = async () => {
    ...
    setLogs((prev: string[]) => {
      const next = [...prev, userLine, agentPrefix];
      agentLineIndex = next.length - 1;
      return next;
    });

    // Freshest local knowledge if server fetch fails
    const enabledNow = (lastEnabledRef.current ?? systemEnabled);

    try {
      // 1) Build fresh authoritative snapshot from server and pass it under "system"
      const { systemLine } = await buildSystemLine(enabledNow);

      // STREAM preferred — include snapshot in "system"
      const res = await fetch(`${API_BASE}/api/console/stream`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          prompt: cmd,
          system: systemLine, // ← key fix: use "system", not "context"
        }),
      });
      ...
      if (!res.ok || !res.body) {
        const r = await fetch(`${API_BASE}/api/console`, {
          ...
          body: JSON.stringify({
            prompt: cmd,
            system: systemLine, // ← keep using "system" here too
          }),
        });
        const data = await r.json();
        if (!r.ok || !data?.ok) throw new Error(data?.error || `HTTP ${r.status}`);

        // Apply side-effect immediately if provided
        if (data.sideEffect?.systemEnabled !== undefined && typeof onSystemToggle === 'function') {
          onSystemToggle(Boolean(data.sideEffect.systemEnabled));
        }

        updateLogAt(agentLineIndex, agentPrefix + (data.text || '(no output)'));
        setBusy(false);
        return;
      }
      ...
      while (true) {
        ...
        // Handle side-effect lines first: [[SIDE_EFFECT]]{...}
        const lines = chunk.split('\n');
        for (const line of lines) {
          if (!line) continue;
          if (line.startsWith('[[SIDE_EFFECT]]')) {
            try {
              const payload = JSON.parse(line.replace('[[SIDE_EFFECT]]', '').trim());
              if (payload.systemEnabled !== undefined && typeof onSystemToggle === 'function') {
                onSystemToggle(Boolean(payload.systemEnabled));
                // Optional: also log the effect so the audit trail shows it
                appendLog(
                  `[SYSTEM] Side-effect → ${payload.systemEnabled ? 'Online ✅' : 'Offline ⛔'}`
                );
              }
            } catch {
              // ignore malformed side-effect payloads
            }
          } else {
            acc += line;
          }
        ...
        // live update with a cursor while streaming
        updateLogAt(agentLineIndex, agentPrefix + acc + (typingBlink ? ' ▌' : ''));
      }

      // final text after stream completes
      updateLogAt(agentLineIndex, agentPrefix + acc.trim());
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Request failed';
      updateLogAt(agentLineIndex, `[ERROR] ${message}`);
    } finally {
      setBusy(false);
    }
  };
  ...
  const commandsByType = { issued: 47, successful: 46, retries: 3, timeouts: 1, failed: 0 };
  ...
  const agentCommandStats = [
    { agent: 'Agent A', issued: 18, successful: 17, failed: 0, successRate: '94.4%', recommendations: ['Optimize grid validation queries', 'Cache power company API responses'] },
    { agent: 'Agent B', issued: 22, successful: 22, failed: 0, successRate: '100%', recommendations: ['Excellent performance', 'Consider expanding role scope'] },
    { agent: 'Agent C', issued: 7, successful: 6, failed: 1, successRate: '85.7%', recommendations: ['Increase timeout threshold for dispatch commands', 'Add retry logic for FLM dispatch'] },
  ];

  const agentPerformance = [
    { name: 'Agent A', role: 'Power Grid Validator', completion: 94, accuracy: 97.2, failRate: 5.6, overrides: 2, precision: 0.95, recall: 0.92, autoRetrySuccess: 88, tokensUsed: 145230, model: 'GPT-4o', avgCostPerTask: '$0.042', issues: ['Occasional grid API timeouts', 'Cache hit rate could be improved'] },
    { name: 'Agent B', role: 'Outage Monitor', completion: 100, accuracy: 99.1, failRate: 0, overrides: 0, precision: 0.99, recall: 0.98, autoRetrySuccess: 100, tokensUsed: 98450, model: 'GPT-4o-mini', avgCostPerTask: '$0.015', issues: [] },
    { name: 'Agent C', role: 'FLM Dispatcher', completion: 67, accuracy: 85.7, failRate: 14.3, overrides: 5, precision: 0.82, recall: 0.79, autoRetrySuccess: 65, tokensUsed: 67890, model: 'Claude 3.5 Sonnet', avgCostPerTask: '$0.038', issues: ['Delegation disabled - constrained mode', 'Timeout threshold too low', 'Needs improved retry logic'] },
  ];
          ...
          <div className="space-y-3 flex-1 overflow-y-auto">
            ...
            <div
              className={`rounded-lg p-3 border transition-all ${
                systemEnabled ? 'bg-green-500/10 border-green-400/30' : 'bg-red-500/10 border-red-400/30'
              }`}
            >
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  />
                  <span className={`text-sm transition-colors ${systemEnabled ? 'text-green-300' : 'text-red-300'}`}>
              ...
              <p className="text-[10px] text-gray-400 leading-tight">
                {systemEnabled ? 'Agentic System is turn on' : 'Turn on to start system'}
              </p>
            </div>
            ...
            <div className="bg-black/40 rounded-lg p-3 border border-blue-400/20">
              ...
              <div className="mt-2 flex items-end gap-2">
                <textarea
                  ...
                  rows={1}
                  placeholder={busy ? 'Running…' : 'Type any instruction'}
                  disabled={busy}
                ...
                >
                  <motion.span
                    initial={false}
                    animate={{ opacity: busy ? 0.6 : 1, scale: busy ? 0.98 : 1 }}
            ...
            >
              <div className="flex items-center justify-center gap-2">
                {showConnections ? <Eye className="w-4 h-4 text-white" /> : <EyeOff className="w-4 h-4 text-white" />}
                <span className="text-white text-sm">{showConnections ? 'Hide' : 'Show'} Connections</span>
              </div>
            </button>
          </div>
        </motion.div>
              ...
              <div className="overflow-y-auto h-[calc(100%-3rem)]">
                <div className="grid gap-3">
                  {policyDirectives.map((policy) => (
                    <div
                      key={policy.id}
                      className="bg-black/30 rounded-lg p-4 border border-purple-400/20 hover:border-purple-400/40 transition"
                    >
              ...
              <div className="h-[calc(100%-3rem)]">
                <Tabs defaultValue="by-type" className="h-full flex flex-col">
                  <TabsList className="bg-slate-800/50 border border-cyan-400/20 mb-4">
                    <TabsTrigger value="by-type" className="data-[state=active]:bg-cyan-500/20">
                      By Command Type
                    </TabsTrigger>
                    <TabsTrigger value="by-agent" className="data-[state=active]:bg-cyan-500/20">
                      By Target Agent
                    </TabsTrigger>
                  ...
                  <TabsContent value="history" className="flex-1 overflow-y-auto">
                    <div className="space-y-2">
                      {commandHistory.map((cmd) => (
                        <div
                          key={cmd.id}
                          className="bg-black/30 rounded-lg p-3 border border-cyan-400/20 hover:border-cyan-400/40 transition"
                        >
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-3">
                              <span className="text-gray-500 text-xs font-mono">{cmd.timestamp}</span>
                              <span className="px-2 py-0.5 bg-cyan-500/20 border border-cyan-400/30 rounded text-xs text-cyan-300">
                                {cmd.type}
                              </span>
                      ...
                      <div className="grid grid-cols-5 gap-3 mb-4">
                        <div className="bg-black/40 rounded p-3 border border-green-400/20">
                          ...
                          <div className="w-full bg-gray-700 rounded-full h-1.5 mt-2">
                            <div className="h-1.5 rounded-full bg-green-400" style={{ width: `${agent.completion}%` }} />
                          </div>
                        </div>
                        <div className="bg-black/40 rounded p-3 border border-blue-400/20">
                          ...
                          <div className="w-full bg-gray-700 rounded-full h-1.5">
                            <div className="h-1.5 rounded-full bg-blue-400" style={{ width: `${agent.accuracy}%` }} />
                          </div>
                        </div>
                        <div className="bg-black/40 rounded p-3 border border-red-400/20">
                          ...
                          <div className="w-full bg-gray-700 rounded-full h-1.5">
                            <div className="h-1.5 rounded-full bg-red-400" style={{ width: `${agent.failRate}%` }} />
                          </div>
                        </div>
                        <div className="bg-black/40 rounded p-3 border border-yellow-400/20">
                        ...
                        <div className="bg-black/40 rounded p-3 border border-purple-400/20">
                          ...
                          <div className="w-full bg-gray-700 rounded-full h-1.5">
                            <div className="h-1.5 rounded-full bg-purple-400" style={{ width: `${agent.autoRetrySuccess}%` }} />
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4 mb-4">
                        <div className="bg-black/40 rounded p-3 border border-cyan-400/20">
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="w-full bg-gray-700 rounded-full h-1.5">
                              <div className="h-1.5 rounded-full bg-cyan-400" style={{ width: `${agent.precision * 100}%` }} />
                            </div>
                            <div className="flex justify-between items-center">
                            ...
                            <div className="w-full bg-gray-700 rounded-full h-1.5">
                              <div className="h-1.5 rounded-full bg-cyan-400" style={{ width: `${agent.recall * 100}%` }} />
                            </div>
                          </div>
                        </div>
