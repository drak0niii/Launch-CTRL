...
import { getSystem, setSystem, subscribeSystem } from "./services/system";
...
export default function App() {
  ...
  // ---- System power state (hydrated + live via SSE) ----
  const [systemEnabled, setSystemEnabled] = useState(true);

  // PATCH 1: subscribe to SSE first; only apply GET if SSE hasn't emitted yet
  useEffect(() => {
    let stop = () => {};
    let sseHasEmitted = false;

    (async () => {
      // 1) Start SSE — authoritative source of truth
      stop = subscribeSystem((enabled: boolean) => {
        sseHasEmitted = true;
        setSystemEnabled(enabled);
      });

      // 2) Fire a one-time GET for hydration, but ignore if SSE already spoke
      try {
        const s = await getSystem(); // { ok, enabled }
        if (!sseHasEmitted && typeof s?.enabled === "boolean") {
          setSystemEnabled(s.enabled);
        }
      } catch {
        // ignore; SSE will settle the state
      }
    })();
  ...
  // Toggle handler used by Sidebar switch AND by console side-effects
  const handleSystemToggle = useCallback(async (next: boolean) => {
    setSystemEnabled(next); // optimistic
    try {
      await setSystem(next); // persist (SSE will also broadcast)
    } catch {
      // revert on failure
